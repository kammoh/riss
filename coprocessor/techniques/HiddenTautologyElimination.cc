...
#include "HiddenTautologyElimination.h"
...
void HiddenTautologyElimination::elimination_worker(CoprocessorData& data, uint32_t start, uint32_t end, BIG& big, bool doStatistics, bool doLock)
{
    ...
    DOUT(if (config.hte_debug_out > 3) cerr << "c allocate litQueue for " << data.nVars() * 2 << " elements at " << std::hex << litQueue << std::hex << endl;);
    MethodFree<Lit> litQueueFree(litQueue); // will automatically free the resources at a return!
    ...
    for (uint32_t index = start; index < end && !data.isInterupted() ; ++index) {
        ...
        if (true) {
            ...
            if (unit != lit_Undef) {
                DOUT(if (config.hte_debug_out > 1) cerr << "c fond failed literal " << unit << " during marking" << endl;);
                if (doLock) { data.lock(); }
...
void HiddenTautologyElimination::parallelElimination(CoprocessorData& data, BIG& big)
{
    ...
    DOUT(if (config.hte_debug_out > 3) cerr << "c parallel HTE with " << controller.size() << " threads" << endl;);
...
bool HiddenTautologyElimination::hiddenTautologyElimination(Var v, CoprocessorData& data, BIG& big, MarkArray& hlaPositive, MarkArray& hlaNegative, bool statistic, bool doLock)
{
    ...
    for (uint32_t pol = 0; pol < 2; ++ pol) {
        ...
        DOUT(if (config.hte_debug_out > 2) {
            ...
            for (uint32_t k = 0; k < iList.size(); k++) {
                ...
                if (cl.size() == 2) {
                    ...
                    if (remClause) {
                        ...
                        DOUT(if (config.hte_debug_out > 0) cerr << "c [HTE] binary removed clause " << cl << endl;);
        ...
        for (uint32_t j = 0; j < binaryIsize; j++) {
            DOUT(if (config.opt_hteTalk && config.hte_debug_out) cerr << "c mark " << toInt(~binaryI[j]) << " with " << toInt(i) << endl;);
...
Lit HiddenTautologyElimination::fillHlaArrays(Var v, BIG& big, MarkArray& hlaPositive, MarkArray& hlaNegative, Lit* litQueue, bool doLock)
{
    ...
    for (uint32_t pol = 0; pol < 2; ++ pol) {
        ...
        for (uint32_t j = 0 ; j < posSize; ++ j) {
            ...
            *(head++) = imp;
            DOUT(if (config.hte_debug_out > 3) cerr << "c [HTE] write at litQueue head pos " << headPos ++ << endl;);
            headPos ++;
            ...
            while (tail < head) {
                const Lit lit = *(tail++);
                DOUT(if (config.hte_debug_out > 3) cerr << "c [HTE] read from array at " << tailPos << " == " << lit << endl;);
                tailPos++;
                ...
                for (uint32_t k = 0 ; k < kListSize; ++k) {
                    ...
                    if (! hlaArray.isCurrentStep(toInt(kLit))) {
                        ...
                        DOUT(if (config.hte_debug_out > 3)  cerr << "c [HTE] put an element to the queue at position " << (int)(head - litQueue) << " with ptr " << std::hex << head << std::dec << endl;);
                        *(head++) = kLit;
                    }
                }
            }
