...
void riss_build_conflict_map(libriss* solver)
{
    ...
    // std::cerr << "setup conflict map based on : " << finalConflict.size() << " literals" << std::endl;
    for (int i = 0 ; i < finalConflict.size(); ++ i) {
        solver->conflictMap[ toInt(finalConflict[i]) ] = 1;
    }
    // std::cerr << "c final map: " << solver->conflictMap << std::endl;
}
...
// 	std::cerr << "continue search with decision level " << solver->solver->decisionLevel() << std::endl;
    ...
    */
    ...
    {
	// std::cerr << "c check for failed: " << lit << std::endl;
        libriss* solver = (libriss*) riss;
...
//         lit = (lit > 0 ? (lit * 2) - 2: (lit * -2) - 1); // turn usual -n <= l <= n,l!=0 into minisat's 0 <= l < 2n
//	std::cerr << "c actual index: " << lit << " out of " << solver->conflictMap << std::endl;
