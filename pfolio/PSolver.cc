...
bool PSolver::integrateNewClause(vec< Lit >& clause)
{
    bool ret = true;
    for (int i = 0 ; i < solvers.size(); ++ i) {
        bool ret2 = l_False != solvers[i]->integrateNewClause(clause); // if a solver failed adding the clause, then the state for all solvers is bad as well, avoid redundancy check for all but the first solver
        if (pfolioConfig.opt_verbosePfolio) if (i == 0) { cerr << "c parsed clause " << clause << endl; } // TODO remove after debug
        ret = ret2 && ret;
    }
    return ret;
}
